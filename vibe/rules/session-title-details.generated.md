<!-- generated-from: code-note/adapters/session-title-details.md sha256:6069aac426d647fcda685611c21aaadffbcf159a1e917c866711e46e1f403fae -->
...
An explicit user request to rename the selected task supplies change authority even when title-origin metadata is absent; do not ask again for that same action. Without that authority, unknown origin remains `title-origin-unavailable`, distinct from `rename-api-unavailable`: report the unresolved gate instead of silently skipping or declaring the feature absent. A temporary/ephemeral task may be a separate unsupported target class; never substitute its parent or another persistent task to make a rename succeed.
